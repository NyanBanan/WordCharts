...
слов в файле и визуализирует частоту их появления на гистограмме
...
Для выполнения обработки данных в отдельном потоке используется класс **WordFrequencyAnalystThread** (
src/WordFrequencyAnalystThread.hpp .cpp)
Внутри метода run() происходит считывание документа и обработка слов объектами других классов
...
4. Создается поток обработки (**WordFrequencyAnalystThread**) и ему передается созданный прокси\
   Также поток соединяет необходимые обработчики с методами внутреннего **CountClass** - *countChanged(const QString&
   word, quint32 count)* и *newWord(const QString& word, quint32 count)*\
   Во время работы, **CountClass** посылает события классу потока, а тот в свою очередь вызывает необходимые методы
   прокси класса\
   Таким образом через класс потока данные от **CountClass** попадают к прокси классу

   **!!!Важно**
   Прокси класс перемещается в поток при помощи *moveToThread(QThread\*)*\
   Класс-поток берет на себя ответственность за удаление созданного прокси, будьте аккуратны, не используйте
   созданный
   прокси напрямую нигде кроме **WordFrequencyAnalystThread**


5. Далее производятся необходимые сигнал-слот соединения (сигнал -> слот)
    + **поток.finished** -> **поток.deleteLater** для автоматического удаления потока при завершении работы
    + **поток.finished** -> **WordFrequencyAnalyst.onThreadEnd** для установки состояния **WordFrequencyAnalyst** на
      значение STOP по завершении работы потока
    + **поток.progressChanged** -> **WordFrequencyAnalyst.progressChanged** выполняется соединения сигнала с сигналом
      для проброса в Qml события изменения прогресса из потока
      значение STOP по завершении работы потока
    + **поток.errorOccured** -> **WordFrequencyAnalyst.errorOccured** аналогично предыдущему но для проброса ошибок
6. Путь файла очищается от файлового url (file://) чтобы WordStream мог его обработать
7. Вызывается метод для старта обработки в потоке
8. Статус устанавливается в значение WORK
...
5. Если не была поставлена пауза (_*pause_required*) или запрошена остановка (*_stop_required*) то после
   непродолжительного sleep все повторяется, пока **WordStream** не опустеет
...
класса **WordFrequencyAnalystThread**, поддерживает в основной модели не более указанного количества самых часто
